    ...
    <div className="flex h-full flex-col">
      ...
      <div className="shrink-0 px-3 pb-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-md border border-input bg-background px-8 py-1.5 text-xs transition focus:border-ring focus:outline-none"
          ...
          <button
            ...
            className={cn(
              'absolute top-1/2 right-2 -translate-y-1/2 rounded p-0.5 transition',
              ignoreCase
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-2 text-xs">
        {!workspaceId ? (
          ...
          <div className="px-2 py-6 text-center break-words text-destructive">
            {error}
          </div>
        ) : showEmpty ? (
          <div className="py-6 text-center text-muted-foreground">
          ...
          <div className="space-y-3">
            {grouped.map(([path, matches]) => (
              <div key={path}>
                <div className="mb-1 truncate font-medium text-muted-foreground" title={path}>
                  {path}
                </div>
                  ...
                  {matches.map((r, i) => (
                    <li key={`${r.path}:${r.lineNumber}:${i}`}>
                      <button
                        ...
                        className="group flex w-full gap-2 rounded px-1.5 py-1 text-left transition hover:bg-primary/10"
                      >
                        <span className="shrink-0 text-muted-foreground tabular-nums group-hover:text-primary">
                          {r.lineNumber}
                        </span>
                        <span className="max-h-12 overflow-hidden font-mono text-[11px] break-all whitespace-pre-wrap text-foreground">
                          {renderLine(r)}
                        </span>
                      </button>
                    </li>
...
function renderLine(r: SearchResult): React.ReactNode {
  ...
  return (
    <>
      ...
      <mark className="rounded bg-yellow-500/30 px-0.5 text-foreground">
