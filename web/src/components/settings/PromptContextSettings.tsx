...
export function PromptContextSettings() {
  ...
  return <section id="prompt-context" className="scroll-mt-4 space-y-6">
    <div className="flex items-center gap-2"><Settings className="size-4 text-muted-foreground" /><h3 className="text-base font-semibold text-foreground">Prompt Context</h3></div>
    <div className="space-y-3 rounded-lg border border-border bg-panel p-4">
      <div><h4 className="text-sm font-semibold text-foreground">Prompt context</h4><p className="mt-1 text-xs text-muted-foreground">Only relative paths are added automatically. File contents are never added from open tabs; explicit editor selections remain separate context.</p></div>
      ...
      {promptContextLoading || !promptContext ? <p className="text-xs text-muted-foreground">Loading context settings…</p> : <>
        ...
        <div className="flex items-center gap-2"><button type="button" onClick={savePromptContext} disabled={promptContextSaving || JSON.stringify(promptContext) === JSON.stringify(promptContextOriginal)} className="rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground transition hover:bg-primary/90 disabled:opacity-50">{promptContextSaving ? 'Saving…' : 'Save context limits'}</button><span className="text-[11px] text-muted-foreground">0 disables a list; maximum 100.</span></div>
      </>}
    </div>
  </section>
}
