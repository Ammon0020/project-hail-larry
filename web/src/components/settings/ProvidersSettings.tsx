...
export function ProvidersSettings({ active, activeSessionId }: { active: boolean; activeSessionId?: string | null }) {
  ...
  return <section id="providers" className="scroll-mt-4 space-y-6">
    <div className="flex items-center gap-2"><Settings className="size-4 text-muted-foreground" /><h3 className="text-base font-semibold text-foreground">Providers (advanced)</h3></div>
    <div className="space-y-3 rounded-lg border border-border bg-panel p-4">
      <div className="flex items-center gap-2"><h4 className="text-sm font-semibold text-foreground">Providers (advanced)</h4>{providersStatus === 'loading' && <span className="text-xs text-muted-foreground">Loading…</span>}</div>
...
function ProviderRow({ provider, busy, error, onSet, onDisable }: { provider: ProviderInfo; busy: boolean; error: string | null; onSet: (apiType: string, baseUrl: string, headersText: string) => void; onDisable: () => void }) {
  ...
  return <div className="space-y-2 rounded-md border border-border bg-background p-3">
    <div className="flex flex-wrap items-center gap-2"><span className="font-mono text-sm text-foreground">{provider.id}</span>{provider.required && <span className="rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] text-amber-600 dark:text-amber-400">required</span>}{supported.length > 0 && <span className="text-[10px] text-muted-foreground">supports: {supported.join(', ')}</span>}</div>
    ...
    <div className="grid grid-cols-1 gap-2 md:grid-cols-[120px_1fr]"><select value={apiType} onChange={event => setApiType(event.target.value)} disabled={busy || supported.length === 0} aria-label={`${provider.id} api type`} className="rounded-md border border-input bg-background px-2 py-1.5 text-sm disabled:opacity-50">{supported.length === 0 && <option value="">—</option>}{supported.map(type => <option key={type} value={type}>{type}</option>)}</select><input type="text" value={baseUrl} onChange={event => setBaseUrl(event.target.value)} placeholder="https://api.example.com" disabled={busy} aria-label={`${provider.id} base URL`} className="w-full rounded-md border border-input bg-background px-3 py-1.5 text-sm disabled:opacity-50" /></div>
    <textarea value={headersText} onChange={event => setHeadersText(event.target.value)} rows={2} disabled={busy} placeholder={'Optional headers (one per line):\nAuthorization: Bearer ...'} aria-label={`${provider.id} headers`} className="w-full rounded-md border border-input bg-background px-3 py-1.5 font-mono text-xs disabled:opacity-50" />
    {error && <ErrorNote message={error} mono />}
    <div className="flex items-center gap-2"><button type="button" onClick={() => onSet(apiType, baseUrl.trim(), headersText)} disabled={!canSet} className="rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground transition hover:bg-primary/90 disabled:opacity-50">{busy ? 'Saving…' : 'Set'}</button>{provider.required ? <span className="text-[10px] text-muted-foreground">Required providers cannot be disabled.</span> : <button type="button" onClick={onDisable} disabled={!canDisable} className="rounded-md border border-input bg-secondary px-3 py-1.5 text-xs font-medium text-foreground transition hover:bg-accent disabled:opacity-50">Disable</button>}</div>
  </div>
}
