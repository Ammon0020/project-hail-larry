...
1. List `docs/plans/` before opening plans; status-prefixed epic folders reveal the portfolio state.
...
- Name work epics `<status>-<epic>/` and stories `<status>-<story>-<urgency>-<difficulty>.md`. Use `pending`, `active`, `blocked`, or `complete` as the status.
- Keep `epic.md` inside its epic folder. Put stories directly in that folder or in its `stories/` subfolder; use one layout consistently within an epic.
