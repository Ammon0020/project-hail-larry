...
description: Create, discover, update, and complete this project's epic and story plans using status-visible filenames. Use when planning work, choosing the next epic or story, updating plan status, or working under plans/.
---
...
1. List `plans/` before opening plans; status-prefixed epic files reveal the portfolio state.
...
- Epics: `<status>-<epic>-<difficulty>.md` at the top level of `plans/`, with a sibling `<epic>/` folder holding its stories.
- Stories: `<status>-<story>-<difficulty>.md` inside the epic's folder.
- Non-epic tasks (bugs, chores): `<status>-<task>-<difficulty>-<urgency>.md` inside `plans/other_tasks/`.
