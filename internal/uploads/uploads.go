...
// MIME types returned by detectImage, extracted as constants to satisfy goconst.
const (
	mimePNG  = "image/png"
	mimeJPEG = "image/jpeg"
	mimeGIF  = "image/gif"
...
// detectImage uses the standard library content-type sniffer to identify the
// image format and returns its MIME type and a canonical extension (with leading
// dot) for a supported image format, or ("", "") if the format is not recognized.
