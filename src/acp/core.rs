...
use super::providers::{require_providers_supported, SessionCaps, MODEL_SWITCH_UNSUPPORTED_MSG};
use super::{
    context::PromptPipeline,
...
use actor::{ActorCommand, ACTOR_COMMAND_CAPACITY};
use diagnostics::StderrTail;
use events::append_payload;
pub use registry::SessionState;
use registry::{SessionEntry, SessionRegistry};
...
// Unit tests exercise the same watchdog without waiting ten seconds per case.
#[cfg(test)]
const CANCEL_GRACE_PERIOD: std::time::Duration = std::time::Duration::from_millis(50);
...
impl Client {
    ...
    pub fn load_conversations(&self) -> Result<(), AppError> {
        ...
        let count = self.sessions.load_dormant(records)?;
        tracing::info!(
            count,
    ...
    fn watch_actor_terminal(
        ...
        tokio::spawn(async move {
            ...
            let current = sessions.mark_failed_if_current(&session_id, handle.id());
            if !current {
                return;
            }
    ...
    fn session_for_command(
        ...
        self.sessions.command(session_id)
    }
    ...
    fn session_for_providers(
        ...
        self.sessions.provider_command(session_id)
    }
    ...
    fn session_for_model_switch(
        ...
        self.sessions.model_command(session_id)
    }
    ...
    fn update_state_if(&self, session_id: &str, expected: SessionState, state: SessionState) {
        self.sessions.update_state_if(session_id, expected, state);
    }
...
impl ACPClient for Client {
    ...
    async fn send_prompt(
        ...
        // Cancel resolves the local prompt immediately, but it does not prove
        // that the agent stopped its remote turn. Keep Interrupted intact so
        // the grace watchdog can kill a process that ignores cancellation.
        // Genuine prompt failures still release the Running slot.
    ...
    async fn cancel_session(&self, session_id: &str) -> Result<(), AppError> {
        ...
        tokio::spawn(async move {
            ...
            let Some(removed) = sessions.take_interrupted(&session_id_owned) else {
                return;
            };
            let sender = removed.commands();
            let _ = sessions.remove_dormant(&session_id_owned);
    ...
    async fn close_session(&self, session_id: &str) -> Result<(), AppError> {
        ...
        if !self.has_live_session(session_id)? {
            let removed = self.sessions.remove_dormant(session_id)?;
            if removed.is_none() {
...
mod tests {
    ...
    use super::{AgentRegistry, Client, ClientDeps, ConversationStore};
    ...
    #[derive(Default)]
    pub(super) struct RecordingPermissions {
        pub(super) cleared_sessions: Mutex<Vec<String>>,
    }
    ...
    pub(super) async fn mock_client() -> (Arc<Client>, Arc<RecordingPermissions>, TempDir) {
    ...
    pub(super) async fn wait_until_running(client: &Client, session_id: &str) {
