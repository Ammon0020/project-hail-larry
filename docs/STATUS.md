...
- **07-15** — Rust fsutil + library guidance: added `src/fsutil` (`dirs` home_dir + `atomic-write-file` durable write with optional Unix mode + parent fsync); config save + logging home resolution use it; deps `dirs` 6.0.0 / `atomic-write-file` 0.3.0. Updated incomplete port stories (S-SEARCH prefer `rg --json`, S-FILES `lru`/`dashmap`, S-MCP/S-PAIRING/S-CONFIG use `fsutil`, S-SYNC dashmap, S-SERVER `rcgen`, S-FSWATCH debouncer, S-PATHUTIL keep custom) and `docs/rust-ecosystem/*`. `cargo fmt`/`clippy -D warnings`/`test` clean (43 lib + 7 spike).
