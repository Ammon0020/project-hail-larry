...
- [ ] **Missing workspace user warning** — Don’t auto-remove missing paths from config; show UI/CLI warning instead (temp prune in `daemon.go`). Story: `docs/plans/stories/pending-missing_workspace_user_warning-med-med.md`.
...
- **07-15** — Daemon start resilience: free orphaned port holders (killed stale `/tmp/local-agent-go`), removed doubled seed-workspace path from user config, `app start` now refuses if PID exists or HTTP/HTTPS ports already bound (`daemon.EnsurePortsFree`), `IsRunning` trims PID whitespace, startup drops missing workspace paths from config after WARNING. `./build.sh` clean; daemon listens 7337/7338.
