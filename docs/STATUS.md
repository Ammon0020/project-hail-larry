...
- [ ] **Rust backend port** — Phase 0 in progress: S-ARCH done, S-CONTRACT done (Go golden fixtures captured), S-ACP-SPIKE done (ACP Rust SDK `agent-client-protocol` v1.2.0 verified against mockagent — initialize/session/prompt/stream/cancel/auth shape all pass; MCP-over-ACP `mcp/message` gap CLOSED via `unstable_mcp_over_acp` feature, dropping the Go inline workaround; no `ExecuteCommand` — shell is `terminal/*`; real-agent E2E + PKCE deferred to S-ACP-CORE). Phase 1 started: S-PATHUTIL done (`src/pathutil/mod.rs` ports `pathutil.SafeJoin` + `workspace.resolveSymlinks` — `clean_path` lexical traversal rejection + `resolve_symlink` on-disk symlink containment; 20 unit tests, tempfile-isolated); S-CONFIG done (`src/config/{error,model,store,tests}.rs` ports `internal/config` — `Config`/`AgentInfo`/`AgentModel` with serde `camelCase` rename matching Go JSON tags, TOML on-disk format (replaces Go `config.json`; S-MIGRATE handles JSON→TOML), atomic write (temp→fsync→chmod 0600→rename→fsync parent), `LOCAL_AGENT_STATE_DIR` override, legacy `tlsEnabled`/`revocationGracePeriodSeconds` secure-by-default key-presence defaulting, unknown-field preservation via `#[serde(flatten)] extra: toml::Table`, `ConfigStore` RwLock wrapper; 13 tests incl. golden DTO match + 0600 perms). Next: remaining Phase 1 stories.
