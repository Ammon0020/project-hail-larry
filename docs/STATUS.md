...
## Bug Fixes & Features (2026-06-27)

- ✅ **Conversations lost on daemon restart (CRITICAL)** — `CloseAllSessions` (`internal/acp/acp.go`) previously called `CloseSession` per session, which `delete(c.sessions, id)` + `persistLocked()` wiped `conversations.json` to empty on shutdown. Added `closeTransportLocked(ctx, sessionID)` which closes the ACP transport (best-effort `DeleteSession` + `Close`), clears permission policies, sets `transport = nil` and `Status = "idle"`, but does NOT remove the session from the map. `CloseAllSessions` now sweeps under a single lock using `closeTransportLocked` and calls `persistLocked()` once at the end, so session metadata survives restart. User-initiated `CloseSession` (UI delete) unchanged — still removes the record. Regression test `TestCloseAllSessionsPreservesMetadata` in `internal/acp/conversation_test.go` verifies metadata survives close + reload; `TestCloseAllSessions` in `lifecycle_test.go` updated to assert preservation + idle status.
- ✅ **Editor tabs horizontal scroll affordances** — Tab bar (`web/src/components/EditorPane.tsx`) replaced `hide-scrollbar` with a thin `.tab-scrollbar` CSS class (`web/src/index.css`, 3px webkit thumb + `scrollbar-width: thin`). Added left/right `ChevronLeft`/`ChevronRight` arrow buttons that appear only when the tab list overflows (`scrollLeft > 0` / `scrollLeft < scrollWidth - clientWidth`); clicking scrolls ~150px. Scroll state updated via `onScroll` handler + `requestAnimationFrame` re-measure on tab count change (no setState-in-effect).
- ✅ **Agent messages rendered as markdown** — `web/src/components/ChatMessageItem.tsx` `StreamUpdate` (non-thought) branch now uses `react-markdown` + `remark-gfm` (GFM) instead of plain `<p>`. Installed `react-markdown@10.1.0`, `remark-gfm@4.0.1`, and `@tailwindcss/typography@0.5.20` (Tailwind v4 `@plugin` directive in `index.css`). Styled via `prose prose-sm prose-invert max-w-none break-words` with arbitrary-variant overrides for `pre` (`bg-tool-call`), inline `code` (`bg-muted`), and links (`text-blue-400 hover:underline`). Streaming cursor preserved.
