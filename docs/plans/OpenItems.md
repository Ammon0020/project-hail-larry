...
- [x] **ACP spec compliance (P1+P2+P4 near-term)** — Deviations and gaps from the audit are implemented. MCP provisioned end-to-end (inline transport). AdditionalDirectories (P4.5) + MCP health UX shipped 2026-07-12. Provider management (P4.11) shipped 2026-07-12. MCP-over-ACP (P4.10) investigated 2026-07-13 → **blocked on SDK** (`mcp/message` not code-generated in acp-go-sdk v0.13.5; can't be wired via stock `ClientSideConnection`) — inline transport retained, blocker + drop-in design in `docs/plans/acp-spec-compliance.md` §4.10 and `docs/known-issues.md`. Remaining future: elicitation, NES, session fork/resume/close, audio, ACP-inspector. Plan: `docs/plans/acp-spec-compliance.md`.
