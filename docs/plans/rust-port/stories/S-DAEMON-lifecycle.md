...
> **Phase:** 4 | **Depends on:** S-SERVER | **Go source:** `internal/daemon/` (773 lines)
...
- `Daemon` struct holding `Arc` references to all managers
- `Start`: initialize all managers in dependency order, start server
...
- [ ] Daemon starts all managers in correct order
