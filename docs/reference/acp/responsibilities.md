...
- **Permission policy** — ✅ Implemented: `allow_always` / `allow_session` / `reject_always` decisions are cached in session-scoped policy maps keyed by `(sessionID, tool, target)` for file-oriented tools and `(sessionID, tool, command)` for shell/execute tools (target is the discriminator for file tools, command for shell tools). This split closes the shell-command bypass where a single `allow_always` for one command would otherwise auto-approve every subsequent shell command in the session. Allow decisions auto-resolve subsequent identical requests without re-prompting; `reject_always` auto-denies without re-prompting. `allow_once` / `reject_once` still prompt every time. Policies are dropped via `ClearSession` when a session closes.
- **Context injection** — ✅ Optional, bounded hints are sent as structured `resource` ContentBlocks when the agent advertises `embeddedContext`, with text fallback for agents that do not. The first prompt may include the configured number of top-level relative workspace paths and `AGENTS.md`; every prompt includes only bounded relative open/recent file paths. An explicit editor selection remains an opt-in resource. Workspace root, platform, time, git status, and automatic file contents are omitted: the agent can request needed data through the negotiated filesystem and terminal callbacks. Profile instructions are injected only when the agent does not advertise ACP mode configuration.
